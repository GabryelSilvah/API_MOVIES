...
tp_url = tp_url.get('tp')

//Fazendo requisição dos dados da API
async function getApi(tipo) {
    try {
        const response = await fetch(`https://api.themoviedb.org/3/tv/${tipo}?api_key=${apiKey}&language=pt-br`)
...
if (tp_url == "popular") {
    tipo_serie = "popular"
} else if (tp_url == "no-ar") {
    tipo_serie = "on_the_air"
} else if (tp_url == "exibindo-hoje") {
    tipo_serie = "airing_today"
...
function criaEstruturaHtml(dados) {
    dados.results.forEach((valor) => {
        ...
        box_filmes.setAttribute("name", valor.name)
